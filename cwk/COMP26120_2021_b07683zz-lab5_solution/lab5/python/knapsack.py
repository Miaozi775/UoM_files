...
class knapsack:
    ...
    def print_instance(self):
        ...
        print("Capability:%d" % self.Capacity)
    ...
    def sort_by_ratio(self):
        ...
        for i in range(1, self.Nitems + 1):
            ...
            print("%f" % (self.item_weights[self.temp_indexes[i]] / self.item_values[self.temp_indexes[i]])) # why not the value/weight???
    ...
    def check_evaluate_and_print_sol(self, sol):
        ...
        for i in range(1, self.Nitems + 1):
            if (pack[i]):
                if (not self.QUIET):
                    print("%d " % self.temp_indexes[i], end="")
                    # print("%d " % i, end="")
