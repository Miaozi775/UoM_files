...
class struc_sol:
    def __init__(self):
        ...
        # objects of this class will also have self.val, self.bound and self.fixed for the value,
        # upper bound of the solutoin and number of items fixed to either True (1) or False (0), not '*"
...
class bnb(knapsack):
    ...
    def print_sol(self,sol):
        ...
        print("sol.val:%d\t  sol.bound: %g\t  \n" % (sol.val,sol.bound), end="")
    ...
    def branch_and_bound(self, final_sol):
        ...
        # branch and bound
        temp_sol= struc_sol()
        self.frac_bound(temp_sol,0)
        current_best=temp_sol.val
        self.insert(temp_sol)
        ...
        # YOUR CODE GOES HERE
        temp_sol.fixed=0
        while(self.QueueSize >0 and temp_sol.bound>current_best and temp_sol.fixed<=self.Nitems):
            temp_sol=self.removeMax()
            temp_sol.fixed += 1
            temp_sol1=temp_sol.copy()
            temp_sol0=temp_sol.copy()
            temp_sol1.solution_vec[temp_sol.fixed]=True
            temp_sol0.solution_vec[temp_sol.fixed]=False
            if(self.frac_bound(temp_sol1, temp_sol.fixed) is  None):
                # print('1')
                if(temp_sol1.val>current_best):
                    # print('11')
                    current_best=temp_sol1.val
                    self.copy_array(temp_sol1.solution_vec,final_sol)
                self.insert(temp_sol1)
            # self.print_sol(temp_sol1)
            if (self.frac_bound(temp_sol0, temp_sol.fixed) is  None):
                # print('0')
                if (temp_sol0.val > current_best):
                    # print('00')
                    current_best = temp_sol0.val
                    self.copy_array(temp_sol0.solution_vec,final_sol)
                self.insert(temp_sol0)
            print(current_best)
            # self.print_sol(temp_sol0)
        # print(current_best)
